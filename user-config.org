...
Disable which-key mode it's nice at first but super slowing down the GUI
#+BEGIN_SRC emacs-lisp
  (which-key-mode -1)
#+END_SRC

Increase spacemacs large files size to 50MB
#+BEGIN_SRC emacs-lisp
  (setq dotspacemacs-large-file-size 50)
#+END_SRC

Devserver: use SSH forwarding
#+BEGIN_SRC emacs-lisp
(defun my-refresh-ssh-auth-sock ()
  "Reset the SSH_AUTH_SOCK environment variable"
  (interactive)
  (setenv "SSH_AUTH_SOCK" (shell-command-to-string "readlink -f -n ~/.ssh/ssh_auth_sock"))
  (message (getenv "SSH_AUTH_SOCK")))

;; Call it right away to setup the env.
(my-refresh-ssh-auth-sock)
#+END_SRC
** window purpose
#+begin_src emacs-lisp
...
Do not filter magit status buffers
#+begin_src emacs-lisp
  (setq spacemacs-useless-buffers-regexp '("magit-.*: .*"))
#+end_src


Magit changed the variables/arguments handling.
Create a custom var to set my options.
#+BEGIN_SRC emacs-lisp
  (defvar transient-default-values '((magit-commit "--gpg-sign=0x32B3CAA03851CBA8")
                                     (magit-log:magit-log-mode "-n64" "--decorate")
                                     ))
  (setq transient-values transient-default-values)
#+END_SRC

Set line wrapping at 70 columns as suggested by the linux kernel
#+BEGIN_SRC emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (defvar magit-blacklist-status-sections-hook
    (seq-filter (lambda (elem)
                  (not (member elem '(magit-insert-untracked-files
  ...
  (defun magit-push-to-gerrit ()
    ...
    (let (target)
      (if (string= (magit-get-current-branch) "master")
          (setq target "master")
        (if (string= (magit-get-current-branch) "main")
            (setq target "main")
          (setq target (completing-read "Target branch:" '("master" "main") ))
          )
        )
      (magit-git-command (concat "git push origin HEAD:refs/for/" target))
      )
    )
...
git-gutter+ is slowing down magit-refresh a lot remove the all-buffers hook
#+begin_src emacs-lisp
  (remove-hook 'magit-pre-refresh-hook 'spacemacs//git-gutter+-refresh-in-all-buffers)
#+end_src
* Diff-mode
** Whitespace-mode
...
#+begin_src emacs-lisp
  ...
  (setq lsp-pylsp-plugins-jedi-environment "/home/loyoll01/.virtualenvs/default/")
  (setq lsp-pylsp-plugins-flake8-config "/home/loyoll01/.config/flake8")
...
Delete files - don't move them to trash
#+begin_src emacs-lisp
...
Optimize startup time by pre-compiling this user-config.org file on save
#+begin_src emacs-lisp
  (defun my/tangle-dotfiles ()
    "If the current file is this file, the code blocks are tangled"
    (when (equal (buffer-file-name) (expand-file-name "~/.spacemacs.d/user-config.org"))
      (org-babel-tangle nil "~/.spacemacs.d/user-config.el")
      (byte-compile-file "~/.spacemacs.d/user-config.el")))
...
(use-package ag
  ...
  ;; This sometimes breaks the goto-file if set to t
  (setq ag-group-matches nil)
  ;; Follow symlinks
  ;; (add-to-list 'ag-arguments "-f")
)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (rg-define-search my-rg-project
    "Run ripgrep in current project searching for thing at point in all files.
    "
    :files "everything"
    :dir project
    :flags ("--no-ignore" "--glob=!ucd_checker.json")
    :confirm prefix)

  (rg-define-search my-rg-default-dir
    "Run ripgrep in default directory searching for thing at point in all files.
    "
    :files "everything"
    :dir current
    :flags ("--no-ignore" "--glob=!ucd_checker.json")
  ...
  (add-to-list 'rg-custom-type-aliases '("mds" . "*.sv *.svh .*.mds"))

  (spacemacs/set-leader-keys
    "sar" 'my-rg-project
    "saR" 'my-rg-default-dir
    )
#+end_src
* lisp funcs
...
(defun smime-customizations ()
  ...
  (setq smime-render-dirs '("/arm/projectscratch/pd/pj02794_matterhorn/loyoll01/reg/mth/simulation/core_tb/Makalu_CoreTB_compile_link"))
  (setq smime-module "/arm/projectscratch/pd/pj02794_matterhorn/loyoll01/reg/mth/misc/modules/midas")
...
* Tarmac
#+begin_src emacs-lisp
  (if (file-directory-p "/work/loyoll01/workdir-6/tarmac-mode")
      (progn
        (normal-top-level-add-to-load-path '("/work/loyoll01/workdir-6/tarmac-mode"))
        (use-package tarmac-mode)
        (evil-set-initial-state 'tarmac-mode 'normal)))
#+end_src
...
* Midas
#+begin_src emacs-lisp
;; please keep the following synchronized with the midas version you are using
(add-to-list 'load-path "/arm/warehouse/IPSolutions/midas/1.2/10/etc/SMIME")
(require 'smime)
#+end_src
