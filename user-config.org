  ...
  (defun my-persp-filter-function (buffer)
    ...
    (or
     (with-current-buffer buffer
       (memq major-mode '(magit-status-mode magit-log-mode magit-revision-mode magit-process-mode)))
...
It is now stored in a file.
TODO think of a way to use a list instead of a local file.
#+BEGIN_SRC emacs-lisp
  (setq transient-values-file (concat dotspacemacs-directory "/transient/values.el"))
#+END_SRC
